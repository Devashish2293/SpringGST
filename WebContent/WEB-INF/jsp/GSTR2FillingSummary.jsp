...
<div class="header-container">
	...
	<div id="gst-home_gstr-dashboard_gstr-gstin_gstr-sales"
		...
		<div class="upload-summary list_page">
			...
			<div class="heading">
				<div class="items--left">GSTR2 Filling Summary</div>
			</div>
			<div class="items--right">
				<button class="button" id="file-GSTR-1-btn"
					style="background-color: rgb(0, 166, 84);">File GSTR-2</button>
				...
				<table>
					<thead>
						<tr>
							...
							<th class="text-right">THROUGH E-COM(Rs)</th>
							<th class="text-right">REV.CHARGE(Rs)</th>
							<th class=""></th>
						</tr>
					</thead>
					<tbody>
						<c:forEach items="${gstr1SummaryData}" var="gstr1SummaryData">
							<tr>
								<td>${gstr1SummaryData.invoiceType.text()}</td>
								<td class="text-right">${gstr1SummaryData.noOfInvoice}</td>
								<td class="text-right">${gstr1SummaryData.taxableAmount}</td>
								<td class="text-right">${gstr1SummaryData.taxAmount}</td>
						...
						<table>
							...
							<tbody>
								<c:forEach items="${gstr1FillingData}" var="gstr1FillingData">
									<tr>
										<td>${gstr1FillingData.invoice_item_id}</td>
										<td></td>
										<td></td>
										<td>${gstr1FillingData.item_id}</td>
										<td>${gstr1FillingData.invoice_id}</td>
										<td >${gstr1FillingData.quantity}</td>
										<td></td>
										<td class="text-right">${gstr1FillingData.total_price}</td>
										<td class="text-right">${gstr1FillingData.igst_total}</td>
										<td class="text-right">${gstr1FillingData.cgst_total}</td>
										<td class="text-right">${gstr1FillingData.sgst_total}</td>
										<td class="text-right">${gstr1FillingData.cess_total}</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
